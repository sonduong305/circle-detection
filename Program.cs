    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (string file in Directory.EnumerateFiles(path, "*.bmp"))
            {
                ...
                {
                    Console.WriteLine("\t [" + circle.Center + " \t R = " + circle.Radius + " \t score = " + Circle_score(circle, img)+" ]");
                    img1.Draw(circle, new Gray(0), 3);
        ...
        static List<CircleF> FindCircle_contours(Image<Gray, Byte> image, int estimatedRadius)
        {
            ...
            //dilated = new Mat().ToImage<Gray, byte>();
            ...
            CvInvoke.FindContours(eroded, contours, hierarchy, RetrType.Tree, ChainApproxMethod.ChainApproxSimple);
            ...
            for (int i = 0; i < contours.Size; i++)
            {
                using (VectorOfPoint contour = contours[i])
                {
                    ...
                    if (isSquare(r.Width, r.Height))
                    {
                        ...
                        if ((Math.Abs(rect_area - circle_area) < rect_area / 10) && (Math.Abs(Math.Sqrt(circle_area/3.14) - estimatedRadius) < 100))
                        {
                            circles.Add(circle);
        ...
        static double Circle_score(CircleF circle, Image<Gray,byte> image)
        {
            ...
            CvInvoke.Circle(mask, Point.Round(circle.Center), (int) circle.Radius, new MCvScalar(255, 255, 255), -1);
            ...
            CvInvoke.Circle(mask, Point.Round(circle.Center), (int)circle.Radius, new MCvScalar(CvInvoke.Mean(cropped, mask).ToArray()[0]), -1);
